...
function Edit() {
  ...
  const columns: ProColumns<TableTransactionHistory>[] = [
    ...
    {
      dataIndex: 'address',
      title: intl.formatMessage({ id: 'pages.broker.address' }),
      width: 400,
      render: (dom, record) => {
    ...
    },
    {
      ...
      title: intl.formatMessage({ id: 'pages.broker.creator_full_name' }),
    },
    {
      dataIndex: 'creator_sale_phone',
      title: intl.formatMessage({ id: 'pages.broker.creator_phone_number' }),
    },
    {
      dataIndex: 'agency',
      title: intl.formatMessage({ id: 'pages.real_estate_sale.cooperation' }),
      render: (dom, record: any) => {
        return <ProFormCheckbox disabled fieldProps={{ checked: !!record?.agency }} />;
      },
    },
  ];
  const onFinish = async (data: any) => {
    ...
    phoneNumberDeleteList = phoneNumberDeleteList?.map((phoneDelete) => {
      return { ...phoneDelete, status: 2 };
    });

    const { data: dataResponse, keyResponse } = await brokerService.updateBrokerById(
      {
        full_name,
        phone_number,
    ...
    if (dataResponse) {
      message.success(
        ...
        () => {
          setPreviosPhoneNumberList(newPhoneNumberList);
  ...
  return (
    <PageContainer
      header={{
        title: intl.formatMessage({ id: 'pages.broker.detail_title' }),
        ghost: true,
        breadcrumb: {
          ...
          routes: [
            ...
            {
              path: '/broker',
              breadcrumbName: intl.formatMessage({ id: 'pages.broker.title' }),
            },
            {
              path: '',
              breadcrumbName: intl.formatMessage({
                id: 'pages.broker.detail_title',
              }),
            },
          ],
        },
    ...
    >
      <ProCard
        headerBordered
        direction="column"
        title={intl.formatMessage({ id: 'pages.broker.info_title' })}
      >
        <ProForm
          onFinish={onFinish}
          formRef={formRef}
          params={{ forceUpdate }}
          request={async () => {
            const { broker_data, phone_number_sub_list, keyResponse } =
              await brokerService.getBrokerById(id, workspace_id);
            if (keyResponse) {
              handleResponseFromCallApi({ response: keyResponse });
              return {};
            }
            const newPhoneList = phone_number_sub_list?.map((phone: { phone_number: string }) => {
        ...
        >
          <Row gutter={24}>
            <Col span={24} xl={{ span: 12 }}>
              <ProFormText
                name="full_name"
                label={intl.formatMessage({ id: 'pages.broker.full_name' })}
                rules={[
                      ...
                      async validator(_re, value) {
                        if (value && value.match(CHECK_PHONE_NUMBER)) {
                          ...
                          if (!response) {
                            ...
                            const duplicateList = phoneNumberList.filter(
                            ...
                            return Promise.reject(
                              new Error(
                                intl.formatMessage({ id: 'form.field.duplicate.phone_number' }),
                              ),
                            );
                          } else
                            return Promise.reject(
                                  ...
                                  rules={[
                                    ...
                                    ({ getFieldValue }) => ({
                                      async validator(_re, value) {
                                        if (value && value.match(CHECK_PHONE_NUMBER)) {
                                          const response =
                                            await brokerService.checkExistPhoneNumber({
                                              broker_id: id,
                                              phone_number: value,
                                            });
                                          ...
                                          if (!response) {
                                            ...
                                            const resultPhoneListRaw =
                                              getFieldValue('phone_number_list');
                                            let phoneNumberList = resultPhoneListRaw.map(
                                              (value: any) => {
                                                let phoneNumber;
                                                _.each(value, (value: any) => {
                                                  if (value.length >= 8) {
          ...
          <h3 className="section-title">
            {intl.formatMessage({ id: 'pages.broker.transaction_history' })}
          </h3>
          <ProTable
            columns={columns}
            rowKey="dealID"
            ...
            request={async (params, sorter) => {
              ...
              const { list, count, keyResponse } = await brokerService.getTransactionHistory({
                broker_id: id,
                offset: offset,
                limit: pageSize || 10,
                sorter: sorter,
