...
import { BROKER_SELL_RENT_VALUE_ENUM } from './constants';
...
function Broker() {
  ...
  useEffect(() => {
    currentUser?.role !== 'sale' &&
      brokerService.getSaleListCreatBroker(workspace_id).then((response) => {
        setSaleList(
          _.isArray(response) &&
            response?.map((value: { id: string; full_name: string; phone_number: string }) => ({
              ...value,
              label: `${value.full_name} - ${value.phone_number}`,
              value: value.id,
            })),
        );
      });
  ...
  const columns: ProColumns<DataSource>[] = [
    ...
    {
      title: intl.formatMessage({ id: 'pages.broker.full_name' }),
      dataIndex: 'full_name',
      hideInSearch: true,
      width: 240,
      render: (dom: any, entity: any) => {
        ...
        return access.brokerEdit ? (
          <Link to={`/broker/${entity.broker_id}`}>
    ...
    {
      title: intl.formatMessage({ id: 'pages.broker.demand' }),
      dataIndex: 'real_estate_type',
      order: 10,
      fieldProps: {
        placeholder: intl.formatMessage({ id: 'global.all' }),
        onChange: (value: string) => {
          tableRef.current?.setFieldsValue({ to_price: undefined });
          tableRef.current?.setFieldsValue({ from_price: undefined });
          if (value) {
            const isDisable = !(value === '1' || value === '2');
            setDisableRangePrice(isDisable);
            const demandTypeNew =
              value === '1'
                ? 'pages.broker.range_price_billion'
                : 'pages.broker.range_price_million';
        ...
        return (
          <>
            {domNew?.map((value: any) => {
              if (Number(value) === 1) {
                return <Tag>{intl.formatMessage({ id: 'pages.broker.sell' })}</Tag>;
              } else if (Number(value) === 2) {
                return <Tag>{intl.formatMessage({ id: 'pages.broker.rent' })}</Tag>;
              }
              return '';
            })}
          </>
      ...
      renderFormItem: () => {
        return <ProFormSelect valueEnum={BROKER_SELL_RENT_VALUE_ENUM} />;
      },
    },
          ...
          <Space size={12} className="custom-form-range">
            ...
            <ProFormDigit
              ...
              rules={[
                ...
                {
                  validator: (rule, value) => {
                    const priceFrom = tableRef.current?.getFieldValue('from_price');
    ...
    },
    {
      ...
      render: (dom, entity: any) => {
        return (
          <Link to={`/broker/${entity.broker_id}`} hidden={!access?.brokerEdit}>
            <LinkTy>
              <svg
                width="16"
                height="16"
  ...
  return (
    <PageContainer
      header={{
        title: intl.formatMessage({ id: 'pages.broker.title' }),
        ghost: true,
        breadcrumb: {
    ...
    >
      <CustomProTableLayout
        ...
        dataTable={{
          ...
          tooltip: intl.formatMessage({ id: 'pages.broker.tooltip.search' }),
        }}
        table={{
          columns: [...columns],
          request: async (params, sorter) => {
            ...
            const { broker_list, count, keyResponse } = await brokerService.getBrokerList({
              province_city_id: province_city_title,
              district_id: district_title,
              creator_sale_id: creator_full_name,
              keyword,
              to_price,
              from_price,
              real_estate_type: real_estate_type && Number(real_estate_type),
              sorter: sorter,
              offset: offset,
              limit: pageSize,
