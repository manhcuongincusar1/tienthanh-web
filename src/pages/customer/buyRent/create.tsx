...
function CreateBuyRent() {
      ...
      async validator(_re: any, value: string) {
        if (value && value.match(CHECK_PHONE_NUMBER)) {
          ...
          if (!response) {
            ...
            let phoneList = getFieldValue('phone_number') ? [getFieldValue('phone_number')] : [];
            const phoneListItem = getFieldValue('phone_number_list')?.map(
              (value: any) => value?.phone_number_item,
            );
            if (phoneListItem?.length > 0) {
              phoneList = [...phoneList, ...phoneListItem];
            }
            const duplicateList = phoneList.filter((phone) => phone === value);
  ...
  const onFinish = async (data: FormData) => {
    ...
    if (isVisiableConfirm) {
      ...
      if (!resultBuy) {
        ...
        message.error(
          intl.formatMessage({ id: 'pages.customer_buy_rent.create_demand_failed' }),
    ...
    if (resultBuy && resultRent && !submitForm) {
      await saveBuyRef?.current?.click();
      await saveRentRef?.current?.click();
      setSubmitForm(true);
      formRef.current?.submit();
    } else if (resultBuy && resultRent && submitForm) {
      const { full_name, phone_number, phone_number_list, goodwill, type_submit } = data;
      const phoneNumberList = phone_number_list?.map((value) => value.phone_number_item);
      const { data: dataResponse, keyResponse } = await customerServices.insertCustomerBuyRent({
        full_name,
        phone_number_main: phone_number,
        phone_number_sub: phoneNumberList,
        goodwill: goodwill,
        data_demand: dataSourceBuy && dataSourceRent && [...dataSourceBuy, ...dataSourceRent],
        branch_id: workspace_id,
      });

      if (dataResponse) {
        message.success(
          intl.formatMessage({ id: 'pages.customer_buy_rent.create_success' }),
        ...
        handleResponseFromCallApi(
          ...
          { localeActionFailedId: 'pages.customer_buy_rent.create_failed' },
        );
      }
    }
  };
  ...
  return (
    <PageContainer
      header={{
        title: intl.formatMessage({ id: 'pages.customer_buy_rent.create' }),
        ghost: true,
        breadcrumb: {
          ...
          routes: [
            ...
            },
            {
              ...
              breadcrumbName: intl.formatMessage({
                id: 'pages.customer_buy_rent.create',
              }),
            },
          ],
        },
            ...
            render(props, dom) {
              return (
                <div className="form-footer">
                  ...
                  <CustomButton
                    onClick={async () => {
                      await formRef.current?.setFieldsValue({ type_submit: 'create_close' });
                      formRef.current?.submit();
                    }}
                    type="primary"
                  >
          ...
          <Row gutter={24}>
            <Col span={24} xl={{ span: 12 }}>
              ...
              <div className={`${Styles.addMore} ${Styles.addMoreFull}`}>
                <Form.List name="phone_number_list">
                  {(fields, { add, remove }) => (
                    <>
                      {fields.map(({ key, name }) => (
