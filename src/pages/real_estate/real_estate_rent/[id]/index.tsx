...
import { Alert, Modal, Col, Row, Dropdown, Button } from 'antd';
import { useIntl, useModel, useParams, useAccess, history } from 'umi';
import FormRealEstateSellEdit from '../../components/FormRealEstateEdit';
...
export default function Create() {
  ...
  const _func = {
    handlePushToPageRealEstateList: () => {
      history.push('/real-estate-rent/list');
    },
    handleRefreshPage: () => {
      setForceUpdate((prv) => prv + 1);
    ...
    onFinish: async (data: any) => {
      const { real_estate_status_id, image_list } = data;
      const response = await checkDuplicateRealEstate(
        formRefRealEstate,
        id,
        REAL_ESTATE_TYPE_ENUM.RENT,
        workspace_id,
      );
      const updateRealEstate = async (duplicate: boolean) => {
        ...
        const responseUpdateRealEstate: string | any =
          await realEstateRentService.updateRealEstateRent(
            {
              ...data,
        ...
        handleResponseEditRealEstate(
          ...
          },
          {
            ...
            localeForbiddenId: `pages.${typePage?.locale || 'real_estate_rent'}.delivered_detail`,
            localeActionFailedId: 'pages.real_estate_sale.edit_failed',
            localeDuplicateId: `pages.${typePage?.locale || 'real_estate_rent'}.duplicated_detail`,
            localeDeleteId: `pages.${typePage?.locale || 'real_estate_rent'}.deleted_detail`,
          },
        );
      };
      if (response) {
        ...
        Modal.confirm({
          ...
          content: (
            <span>
              {intl.formatMessage({ id: 'pages.real_estate_sale.continue_change' })} <br />
              {intl.formatMessage({ id: 'pages.real_estate_sale.save_your_record' })}
            </span>
          ),
          onOk() {
    ...
    handlePushToPageRealEstateRentList: () => {
      history.push('/real-estate-rent/list');
    },
  };
  useEffect(() => {
  ...
  return (
    <PageContainer
      header={{
        title: intl.formatMessage({ id: 'pages.real_estate_rent.detail' }),
        ghost: true,
        breadcrumb: {
          ...
          routes: [
            ...
            {
              path: '',
              breadcrumbName: intl.formatMessage({ id: 'pages.real_estate_rent.detail' }),
            },
          ],
        },
        extra: [],
    ...
    >
                ...
                <Dropdown
                  overlay={
                    <MenuAction
                      handlePushPageToPageList={_func.handlePushToPageRealEstateList}
                      handleRefreshPage={_func.handleRefreshPage}
                      hiddeDuplicateButton={!access.realEstateRentDuplicate || visibleAlert}
                      hiddeAssignButton={!access.realEstateRentAssign}
                      hiddeDeleteButton={!access.realEstateRentDelete}
      ...
      >
        ...
        <FormRealEstateSellEdit
          onFinish={_func.onFinish}
          pathListPage={'real-estate-rent'}
          realEstateService={realEstateRentService}
        />
      </ProCard>
    </PageContainer>
