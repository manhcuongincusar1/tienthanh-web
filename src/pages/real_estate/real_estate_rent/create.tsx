...
export default function Create() {
  ...
  const setDefaultData = () => {
    formRefRealEstate?.current?.setFieldsValue({
      real_estate_status_id: realEstateStatus.find(
        (value) => value.type === REAL_ESTATE_TYPE_ENUM.RENT && value.isDefault,
      )?.value,
    });
  ...
  const onFinish = async (data: any) => {
    const { image_list, type_submit, real_estate_status_id } = data;
    ...
    const previousRealEstateStatus = realEstateStatus.find(
      (value) => value.type === REAL_ESTATE_TYPE_ENUM.RENT && value.value === real_estate_status_id,
    )?.label;
    ...
    const onInsertRealEstate = async (duplicate: boolean) => {
      const { data: dataResponse, keyResponse } = await realEstateRentService.insertRealEstateRent({
        ...data,
  ...
  return (
    <PageContainer
      header={{
        title: intl.formatMessage({ id: 'pages.real_estate_rent.create' }),
        ghost: true,
        breadcrumb: {
          ...
          routes: [
            ...
            {
              path: '',
              breadcrumbName: intl.formatMessage({ id: 'pages.real_estate_rent.create' }),
            },
          ],
        },
        extra: [],
    ...
    >
      <ProCard
        headerBordered
        direction="column"
        title={intl.formatMessage({ id: 'pages.real_estate_rent.info' })}
      >
        <Row>
